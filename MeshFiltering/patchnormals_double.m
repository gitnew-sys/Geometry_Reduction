function [Nx,Ny,Nz,FNormal,Fa,Fb,Fc,Fcenter,Angle]=patchnormals_double(Fa,Fb,Fc,Vx,Vy,Vz)
%
...
%try to find the center of each faces
for i=1:size(Fa,1)
Fcenter(i,:)=(e1(i,:)+e3(i,:))/2 + FV.vertices(Fb(i),:);
end
...
% Calculate normal of face
 Normal=cross(e1,e3);
 FNormal =Normal; % worked idea
%                 % my new idea==> found out problem: fcenter aren't right
% fNormal1 = crossp(e1,e3);
% fNormal2 = crossp(e2,e3);
% FNormal = cross(fNormal1,fNormal2);
